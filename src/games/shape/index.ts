...
export const shape: GamePackage<Board, ShapeState> = {
  meta: {
    ...
    rules: [
      'Every row and every column holds one of each shape. The other cells stay empty.',
      'A clue sits outside the grid and looks in along its row or column. Its number says which shape it means: 1 is the first shape you would meet looking in, 2 is the one after that. Tap any clue and the board reads it out.',
      'Choose a shape under the board, then tap a cell to put it there. Tap it again to take it off, or drag it to another cell to move it.',
      'The × is a note to yourself that a cell is empty. You never have to place one: the board is solved when every shape is where it goes.',
    ],
    accent: 'a-shape',
    shareName: 'Shape Up',
  },
